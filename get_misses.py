...
def main(args):
    ...
    # For data fri]om csv file
        ...
        data_lru = {'LRU Miss PC': pcs_lru,'LRU Miss Address': lru_misses, 'LRU Miss Recency': recencies_lru, 'LRU Miss Frequency': frequencies_lru}

        new_df_lru = pd.DataFrame(data_lru,columns=['LRU Miss PC','LRU Miss Address','LRU Miss Recency','LRU Miss Frequency'])
        new_df_lru.to_csv(Path(f+'.csv').resolve().parents[1].joinpath('misses').joinpath(f.split(args.r)[1] +'_lru_misses.csv'))

        data_lfu = {'LFU Miss PC': pcs_lfu,'LFU Miss Address': lfu_misses, 'LFU Miss Recency': recencies_lfu, 'LFU Miss Frequency': frequencies_lfu}

        new_df_lfu = pd.DataFrame(data_lfu,columns=['LFU Miss PC','LFU Miss Address','LFU Miss Recency','LFU Miss Frequency'])
